...
$('.editcomment').on('click', function(event) {
      ...
      var comment_id = $(this).attr('comment');
      var oldContent = box.find('.commentcontent').val();
      ...
      $('.ok').on('click', function(event) {
        ...
        $.ajax({
            ...
            success:function(response){
              ...
              {
                box.find('.ok').addClass('hide');
                box.find('.editcommentbox').addClass('alert alert-danger');
                box.find('.editcommentbox').val('Sorry, '+response['content']);
                setTimeout(function(){box.find('.editcommentbox').removeClass('alert alert-danger').addClass('hide').val('');},2000)
                box.find('.commentcontent').removeClass('hide');
              }
